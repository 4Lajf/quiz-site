...
export const handleNotes = async (team, mode, notes) => {
    try {
        ...
        if (canAnswer[0].canAnswer === false) {
            toast.pop();
            await sleep(500);
            failure('Time for answering this question ran out.', 5000);
            return;
        }
        ...
        if (isNaN(notes) || notes <= 0) {
            toast.pop();
            await sleep(500);
            failure('Please provide a positive number', 5000);
            return;
        }
        ...
        if (noteFetchArr[0] === 1) {
            await sleep(500);
            failure('The round has ended', 5000);
            return;
        }

        if (mode === 1) {
            notes = noteFetchArr[0] - 1;
            console.log(notes, noteFetchArr[0] - 1)
        }

        if (notes > noteFetchArr[0] - 1) {
            await sleep(500);
            failure('Please provide number smaller than ' + noteFetchArr[0], 5000);
            return;
        }
