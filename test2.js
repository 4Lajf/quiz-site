...
//TODO: accept answer on TAB, loading when fetching
/* 
Scoring Mode:
...
class SongArtistMode {
    ...
    #answerResultsHelper = (value, hashesMap, scoreMap, answerMap, revealFunction) => {
        for (let i = 0; i < playerAmount; i++) {
            console.log('MAP:', answerMap.get(playerData[i]))
        }

        hashesMap.forEach((answer, sender) => {
            console.log('hMAP:', answer, sender)
            if (!submitedArtist) {
                submitedArtist = 'none'
            }
            if (!submitedTitle) {
            ...
            if (scoringMode === 1) {
                ...
                console.log(possibleAnswers, submitedArtist, submitedTitle)
